...
public class GUITextField extends GUIText {
	...
	public void render() {
		...
		if (textWidth < boxWidth)
			font.drawString(getPosition().getX() + guiBg.getBorderSize(), getPosition().getY() + guiBg.getBorderSize(), text.toString(), Util.convertColor(color));
		else
			font.drawString(getPosition().getX() + guiBg.getBorderSize() - (textWidth - boxWidth), getPosition().getY() + guiBg.getBorderSize(), text.toString(), Util.convertColor(color));
		RenderHelper.endScissor();
		if (focused && cursorState)
			RenderHelper.drawSquare(getPosition().getX() + getCursorPos(textWidth, boxWidth), getPosition().getY() + guiBg.getBorderSize() + 2, guiBg.getBorderSize(), dimension.getHeight() - (guiBg.getBorderSize() * 2) - 4, color);
	}
